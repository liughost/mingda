 ...
 -->
	...
	<table class="table">
		<c:forEach var="items" items="${msgList}" varStatus="status">
			...
			<tr>
				<td style="width: 80px; text-align: center;"><c:if
						test="${cust_id != items.customer.id }">
						<a href="${context }/tour/detail?id=${items.customer.id}"><img
							style="width: 80px; height: 80px;"
							src="${items.customer.headImg}"><br />
							${items.customer.nickName } </a>
					</c:if></td>
				<td style="max-width: 500px;">
					<div style="min-height: 80px;">
						<c:choose>
							<c:when test="${items.msgType== 'image'}">
								<img src="${items.content }" style="max-width: 450px;">
							</c:when>
							<c:otherwise>
					...
					<div>
						<span class="tb_date">发表日期：${items.createTime}</span>
					</div>
				</td>
			</tr>
